...
int read_file_data_double(mix2mono_config_t* restrict mix2mono_conf, SNDFILE* restrict sndfile, SF_INFO* restrict sf_info, double** x)
{
    *x = malloc(sf_info->channels * sf_info->frames * sizeof(double));
...
void mix2mono(size_t frames, int channels, double* restrict x, double** restrict x_mono)
{
    *x_mono = calloc(frames, sizeof(double));
    for (uint64_t i = 0; i < frames; i++) {
        for (uint16_t c = 0; c < channels; c++) {
            (*x_mono)[i] += x[i * channels + c]/channels;
        }
    }
}

int write_file(SNDFILE** restrict file, SF_INFO* restrict sf_info, double* restrict x_mono, mix2mono_config_t* restrict mix2mono_conf)
{
    ...
    *file = sf_open(mix2mono_conf->ofile, SFM_WRITE, &osf_info);
    ...
    sf_write_double(*file, x_mono, osf_info.frames * osf_info.channels);
